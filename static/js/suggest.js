...
/////////////////////////////////
  ...
  function highlight(first_date, last_date){
    ...
    if(first_date>last_date) {
      for (let i = last_date + 1; i <= first_date - 1; i++) $(date_box[i]).addClass('middle');
      $(date_box[first_date]).addClass('less');
      $(date_box[last_date]).addClass('more')
    }else {
      for (let i = first_date + 1; i <= last_date - 1; i++) $(date_box[i]).addClass('middle');
  ...
  function crossmonth (){
    if(end_date.month>start_date.month && end_date.year === start_date.year){ // Increasing months, but not year
    ...
    }else if(end_date.year>start_date.year){ // Increasing the year
      highlight(0, end_date.index);
    ...
    }else if(end_date.month<start_date.month && end_date.year === start_date.year){ // Decreasing months, but not year
...
/////////////////////////////////
  ...
  $('.rectangle').on('click', function () {
    ...
    let name = $($('input')[0]).val();
    let website = $($('input')[1]).val();
    let country = $('#country p').text().split('▾')[0];
    let city = $($('input')[2]).val();
