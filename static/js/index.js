$(document).ready(function () {
  ...
  $('#search').on('click', function () {
    ...
    let month = $('#month p').text().split('▾')[0].substring(0, 3);
    let search = '/search?';
  ...
  if (url.includes('search')) {
    let country = url.split('country').pop().split('&')[0].split('=')[1].replace(/%20/g, ' ');
    let title = url.split('title').pop().split('&')[0].split('=')[1];
    let month = url.split('month').pop().split('&')[0].split('=')[1];
