$(document).ready(function() {
  ...
  $('.item.big.admin').each(function() {
    // Input and textarea are self closing elements
    let input = $(this).find('input') // Therefore we can't set values dynamically with pug (you could also use the data field)
    for (let item in input) {
      // So we set their placeholder dynamically, update their val using this script
  ...
  function approve(container) {
    ...
    if (file) filename = file.name
      ...
      type() == 'suggestion' &&
      $(container.find('.img')[0]).css('background-image') !==
        'url("https://designconferenc.es/img/placeholder.png")'
    )
      filename = $(container.find('.img')[0])
